...
export class NotificationService {
  ...
  getUnreadByResponsableId(responsableId: number): Observable<Notification[]> {
    return this.http.get<Notification[]>(`${this.apiUrl}/responsable/${responsableId}/non-lues`).pipe(
  ...
  // Polling pour notifications en temps réel
  startPolling(responsableId: number): Observable<Notification[]> {
    return interval(30000).pipe(
      switchMap(() => this.getUnreadByResponsableId(responsableId))
    );
  }
}
